...
# --- Функция-обертка для запуска проверки изменений через job queue ---
async def check_asset_changes_job(context):
    """Обертка для запуска проверки изменений через job queue"""
    await check_asset_changes(context.application)
...
def main():
    ...
    async def start_monitoring(application):
        application.job_queue.run_repeating(
            callback=check_asset_changes_job,
